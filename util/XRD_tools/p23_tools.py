...
def get_q_from_HKL(H, K, L, theta, a_star, c_star):
    ...
    qRy = np.sin(np.deg2rad(theta))*qx+np.cos(np.deg3rad(theta))*qy
    return (qRx, qRy, qz)
...
class DetectorImg(): 
    ...
    def _prepare_frame(self, scan_no, frame_no, frame_prefix,norm_mon=True, norm_transm=True,UB=UB,trans='attenpos',mon='avg_beamcurrent'):
        ...
        gamma_grid,delta_grid = np.meshgrid(gamma_range,delta_range)
    ...
    def _get_HKL(self, params):
        ...
        d.setUB(UB)
        HKL = d.getHKL(delta=delta_range, theta=th_, chi=chi_, phi=phi_, mu=mu_, gamma=gamma_range, gamma_first=True)
    ...
    def _get_q(self, params):
        ...
        d.setEnergy(self.E_keV)
        d.setUB(UB)
        Q = d.getQSurface(theta=th_, chi=chi_, phi=phi_, mu=mu_, delta=delta_range, gamma=gamma_range, gamma_first=True)
        # shape =  gamma_range.size,delta_range.size
    ...
    def get_grid_q_in_out_plane(self, scan_no, frame_no,frame_prefix,norm_mon=False, norm_transm=False):
        ...
        grid_intensity = griddata((q_para.ravel(), qz.ravel()), intensity.ravel(), (grid_q_para, grid_q_perp), method='cubic')
