...
class space_plot():
    ...
    def plot_peaks(self, HKL_lims=[-10, 10], qx_lims=None, qy_lims=None,
        ...
        for HKL in HKLs:
            ...
            if(self.is_in_limits(q, qx_lims, qy_lims, q_inplane_lim, qz_lims, mag_q_lims)):
                I = self.lattice.I(HKL)/I0 # Normalize to (0,0,0) intensity and plot the sqrt 
                if(I > 1e-3): # only add point to list if it is larger than 1e-3
                    qs.append(q)
                    Is.append(I)
        ...
        Is = np.array(Is)
        if(len(Is > 0)):
            return (mlab.points3d(qs[0]*scale_q[0], qs[1]*scale_q[1], qs[2]*scale_q[2], np.sqrt(Is), scale_factor=scale_factor, color=color), qs, Is, self.lattice)
        return None
    ...
    def get_rod_positions(self, HKL_lims=[-10, 10], qx_lims=None, qy_lims=None,
                          q_inplane_lim=None, qz_lims=None, color=(0, 0, 0), ignore00rod=False):
        HKL_range = np.arange(HKL_lims[0], HKL_lims[1] + 1)
        HKLs = list(itertools.product(HKL_range, repeat=3))
        qIs = []
        # Find all lattice positions within limits
        for HKL in HKLs:
            if(ignore00rod and HKL[0] == 0 and HKL[1] == 0):
                continue
            q = self.lattice.q(HKL)
    ...
    def plot_rods(self, HKL_lims=[-10, 10], qx_lims=None, qy_lims=None,
                  q_inplane_lim=None, qz_lims=[-10, 10], color=(0, 0, 0), plot00rod=True, scale_q=[1., 1., 1.]):
