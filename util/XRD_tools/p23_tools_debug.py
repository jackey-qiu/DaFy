...
class DetectorImg():
    def __init__(self, E_keV=19.5, cen=(234,745), pixelsize=(0.055,0.055), sdd=714, UB=UB,spec_filename='/home/qiu/data/beamtime/P23_11_18_I20180114/raw/startup/FirstTest_00671.fio', edf_path='/home/qiu/data/beamtime/P23_11_18_I20180114/raw/FirstTest_00671/lmbd',motor_angles=None, frame_prefix = 'FirstTest'):
        self.set_params(E_keV, cen, pixelsize, sdd, spec_filename, edf_path,UB, motor_angles, frame_prefix)

    def set_params(self, E_keV=0.0, cen=(0,0), pixelsize=(0,0), sdd=0.0, spec_filename="", nexus_path="",UB=[], motor_angles=None, frame_prefix = 'FirstTest'):
        self.E_keV = E_keV
        ...
        self.sdd = sdd
        if spec_filename!=None:
            self.fio = Fio.Fiofile(spec_filename)
        else:
            self.fio = None
        self.img_loader = nexus_image.nexus_image_loader(spec_filename, nexus_path, frame_prefix = frame_prefix)
        self.UB=UB
    ...
    def _prepare_frame(self, scan_no, frame_no, frame_prefix,norm_mon=True, norm_transm=True,UB=UB,trans='attenpos',mon='avg_beamcurrent',one_frame_in_one_nxs=True):
        self.scan_no = scan_no
        self.frame_no = frame_no
        self.frame_prefix = frame_prefix
        img =self.img_loader.load_frame(scan_no, frame_no,one_frame_in_one_nxs)
        self.UB = UB
        if self.motor_angles == None:
    ...
    def get_HKL(self, scan_no, frame_no, frame_prefix,norm_mon=True, norm_transm=True,one_frame_in_one_nxs=True):
        intensity, params = self._prepare_frame(scan_no, frame_no, frame_prefix,norm_mon=norm_mon, norm_transm=norm_transm,one_frame_in_one_nxs=one_frame_in_one_nxs)
        return (intensity, self._get_HKL(params))

    def get_q(self, scan_no, frame_no, frame_prefix,norm_mon=True, norm_transm=True,one_frame_in_one_nxs=True):
        intensity, params = self._prepare_frame(scan_no, frame_no, frame_prefix,norm_mon=norm_mon, norm_transm=norm_transm, one_frame_in_one_nxs=one_frame_in_one_nxs)
        return (intensity, self._get_q(params))

    def get_HKL_q(self, scan_no, frame_no, frame_prefix,norm_mon=True, norm_transm=True,one_frame_in_one_nxs=True):
        intensity, params = self._prepare_frame(scan_no, frame_no, frame_prefix,norm_mon=norm_mon, norm_transm=norm_transm, one_frame_in_one_nxs=one_frame_in_one_nxs)
        return (intensity, self._get_HKL(params), self._get_q(params))

    def get_grid_q_in_out_plane(self, scan_no, frame_no,frame_prefix,norm_mon=False, norm_transm=False, one_frame_in_one_nxs=True):
        intensity, q = self.get_q(scan_no, frame_no,frame_prefix, norm_mon=norm_mon, norm_transm=norm_transm, one_frame_in_one_nxs=one_frame_in_one_nxs)
        qx, qy, qz = q
...
if __name__ == '__main__':
    ...
    plt.figure()
    plt.pcolormesh(grid_q_para, grid_q_perp, grid_intensity, vmin=0, vmax=190,cmap='jet')
