...
class lattice():
    def __init__(self, a, b, c, alpha=90, beta=90, gamma=90, basis=[0, 0, 0],
        ...
        for i in xrange(len(self.basis)):
            if(isinstance(self.basis[i][0], basestring)):
                f1, f2 = (elements.symbol(str.strip(str(self.basis[i][0]), '123456789')).
    ...
    def from_cif(filename, HKL_normal=[0, 0, 1], HKL_para_x=[1, 0, 0],
                 offset_angle=0, E_keV=22.5, override_a=None, override_b=None, override_c=None, override_alpha=None, override_beta=None, override_gamma=None, use_numbered_atom_labels=False):
    ...
    def F(self, HKL, rod_int=False, rod_alpha=0.1):
        ...
        Q = np.sqrt(q.dot(q))
        for i in xrange(len(self.basis)):
            f = elements.symbol(str.strip(str(self.basis[i][0]), '123456789')).xray.f0(Q)
    ...
    def angles(self, HKL, mu):
        ...
        def f(theta):
            v = q + RotationMatrix(0, np.deg2rad(mu),
