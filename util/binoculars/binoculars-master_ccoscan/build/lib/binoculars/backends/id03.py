...
class ID03Input(backend.InputBase):
    ...
    #TODO: test - modified by F. Reikowski
    @staticmethod
    def is_zap(scan):
        return (scan.command().startswith('zap') or scan.command().startswith('ccoscan'))
    @staticmethod
    def is_ccoscan(scan):
...
class EH1(ID03Input):
    ...
    def get_point_params(self, scan, first, last):
	    ...
	    #TODO: test - added by F. Reikowski
            if self.is_ccoscan(scan):
