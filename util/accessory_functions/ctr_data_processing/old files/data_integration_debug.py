...
"""
if mpi_tag:
    spec_path='/net/filet/team/fwog/members/qiu05/Dec_2017_APS'
    spec_name='s4_zr_10mM_RbCl_1.spec'
    scan_number=range(16,54)#[55]+range(35,50)#+range(35,50)+[55]
    substrate='muscovite'
    comm=MPI.COMM_WORLD
    size=comm.Get_size()
    rank=comm.Get_rank()
...
dl_bl_muscovite={'0_0':{'segment':[[0,18]],'info':[[1,1]]}}
...
HK_CENTER_CHECK=[0,0]
BRAGG_PEAKS_HEMATITE={'3_0':[1,7],'2_0':[0,2,4,6],'2_1':[0.8609,4.8609,6.8609],'2_2':[0,1.7218,3.7218,5.7218,7.7218],'2_-1':[1.1391,3.1391,5.1391],'1_1':[0,1.8609,3.8609,5.8609],'1_0':[1,3,5],'0_2':[0,1.7218,3.7218,5.7218],'0_0':[0,2,4,6,8,10,12,14,16],\
...
def image_process_temp(spec_path='/Users/cqiu/data/APS/Dec_2017_APS/images/s2_zr_100mM_KCl_1/S015/s2_zr_100mM_KCl_1_S015_00203.tif'):
    im_org=misc.imread(spec_path)
    im=copy.copy(im_org)
    #im[:,0:238]=1
    #im[:,265:-1]=5
    #im_filtered=scipy.ndimage.filters.gaussian_laplace(im, sigma=0.6, output=None, mode='reflect')
    im=ndimage.gaussian_filter(im, sigma=2)
    im_filtered=ndimage.morphology.morphological_gradient(im,size=(5,5))
    #im_filtered = gradient(im, disk(5))
...
class data_integration:
    def __init__(self,spec_path='/Users/cqiu/data/APS/Dec_2017_APS',spec_name='s2_zr_100mM_KCl_1.spec',scan_number=range(40,43)+range(44,53)+range(54,76),\
    ...
    def extract_data_info(self):
        ...
        data_info_partial['images_path']=[]
        parced_items=["H","K","L","chi","mu","nu","eta","del","phi","time","norm","images_path",'transmision']
        scans_unique=[]
        ...
        for scan in scans_unique:
            ...
            for item in data_info.keys():
                if item in parced_items_temp:
                    ...
                    #print rank,item,scan_index,image_index
    ...
    def find_center_pix_smart_3(self,scan_index,image_index=0,debug=False):
        ...
        def _bragg_peak_check(offset=1):
            l_lib=self.dl_bl
            ...
            for i in range(len(segment)):
                if l>=segment[i][0] and l<=segment[i][1]:
                    ...
                    if offset_list.min()<=offset:
                        ...
                        return 0,None
        bragg_tag,l_offset_min=_bragg_peak_check()

        if bragg_tag==1:
            if self.integ_pars['integration_direction']=='y':
                current_img[:,int(center_pix[1]+15*l_offset_min/1.):-1]=0
            else:
                current_img[int(center_pix[0]+15*l_offset_min/1.):-1,:]=0
        elif bragg_tag==-1:
            if self.integ_pars['integration_direction']=='y':
                current_img[:,0:int(center_pix[1]-15*l_offset_min/1.)]=0
            else:
                current_img[0:int(center_pix[0]-15*l_offset_min/1.),:]=0
        else:
            if self.integ_pars['integration_direction']=='y':
                current_img[:,int(center_pix[1]+15):-1]=0
            else:
                current_img[:,0:int(center_pix[1]-15)]=0
        if debug:
            plt.figure()
            plt.imshow(current_img)
        current_img[np.where(current_img<(current_img.max()*0.9))]=0
        current_img=ndimage.gaussian_filter(current_img, sigma=2)
        if debug:
            plt.figure()
            plt.imshow(current_img)
        ...
        try:
            ...
            if r_width>35:#large r width means failure of finding the right center due to large noise level, take values from the previous point
        ...
        if mpi_tag:
            ...
            pass
        if debug:
            print "center_pix",int(center_pix[0]),int(center_pix[1])
            return pd.DataFrame(current_img)
