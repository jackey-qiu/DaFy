...
if mpi_tag:
    ...
    scan_number=range(12,33)#[55]+range(35,50)#+range(35,50)+[55]
    substrate='hematite'
    comm=MPI.COMM_WORLD
    size=comm.Get_size()
    rank=comm.Get_rank()
...
off_bragg_peak_sig_noise_ratio_factor={'muscovite':1.5,'hematite':1.5}
...
off_bragg_peak_gaussian_filter_sigma={'muscovite':2,'hematite':4}
    ...
    def find_center_pix_smart_3(self,scan_index,image_index=0,debug=False):
        ...
        if self.substrate=='muscovite':
            if len(l_bragg)>=2:
                ...
                ref_img=misc.imread(self.data_info['images_path'][scan_index][-1])
                center_pix=[np.where(ref_img==np.max(ref_img))[0][0],np.where(ref_img==np.max(ref_img))[1][0]]
        elif self.substrate=='hematite':#in hematite case, misallignment is more severe to make spot move more on pilatus image. And it has much less influence from bragg peaks. So it is safe to use the max spot as the center spot.
        ...
        if debug:
            plt.figure()
            plt.imshow(current_img,vmax=np.max(current_img))
        ...
        try:
            ...
            if abs(center_pix_y-self.integ_pars['center_pix'][0])>bad_image_cutoff[self.substrate]:
        ...
        if mpi_tag:
            ...
            pass
        if debug:
            print (self.integ_pars['c_width'],self.integ_pars['r_width'])
            print (self.integ_pars['center_pix'])
            print ("center_pix",center_pix_x,center_pix_y)
            print ('column width and row width are:',c_width,r_width)
