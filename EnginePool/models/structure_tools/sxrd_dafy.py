...
class Sample:
    def __init__(self, inst, bulk_slab, slabs,unit_cell,surface_parms={'delta1':0,'delta2':0},
        ...
        #self.set_sorbate_sym(sorbate_sym)
        self.inst = inst
    ...
    def set_sorbate_sym(self, sym_list):
        '''Sets the list of allowed symmetry operations for the bulk
    ...
    def make_xyz_file_(self, save_file='xyz.xyz',use_sym = True, trans_opt = [0.5,0.5]):
        for key in self.domain.keys():
            ...
            if save_file_temp.endswith('.xyz'):
                save_file_temp = save_file_temp.replace('.xyz','_{}.xyz'.format(key))
            else:
        ...
        if self.coherence==True:
            for key in self.domain.keys():
                if self.domain[key]['wt']!=0:
                    ftot = ftot+(fb+self.calc_fs(h, k, l,[self.domain[key]['slab']]))*self.domain[key]['wt']
        else:
            for key in self.domain.keys():
                if self.domain[key]['wt']!=0:
                    ftot = ftot+abs(fb+self.calc_fs(h, k, l,[self.domain[key]['slab']]))*self.domain[key]['wt']
        return abs(ftot)*self.inst.inten

    def calc_f_all(self, h, k, l):
        '''
        Calculate the structure factors for the sample
...
class UnitCell:
    ...
    def q_par_q_ver(self, h, k, l):
        return self.lattice.q_par_q_ver(h, k, l)

class lattice():
    def __init__(self, a, b, c, alpha=90, beta=90, gamma=90,
                 HKL_normal=[0, 0, 1], HKL_para_x=[1, 0, 0]):
        self.a = a
        self.b = b
        self.c = c
        ...
        q_normal = self.RecTM.dot(HKL_normal)
    ...
    def q_par_q_ver(self, h, k, l):
        qx,qy,qz = self.q(h, k, l)
        return np.sqrt(np.square(qx)+np.square(qy)), qz
...
class AtomGroup:
    ...
    def __init__(self, slab = None, id = None,matrix=[1,0,0,0,1,0,0,0,1], instance_name = 'instance_name', fit_pars = []):
        ...
        if len(fit_pars)==0:
            ...
            self.fit_pars = [each for each in fit_pars if each in self.par_names]
            self.par_names = self.fit_pars
