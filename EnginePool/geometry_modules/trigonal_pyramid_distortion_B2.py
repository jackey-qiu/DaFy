...
class trigonal_pyramid_distortion():
    ...
    def cal_apex_coor(self,switch=False,phi=0.,mirror=False,angle_offset=0):
    ...
    def cal_p2(self,p0,p1,mirror=False,angle_offset=0):
        ...
        _p2_v=_p2-self.apex
        #the ref len is the shorter length between apex and two known points
        scale=(min([f2(self.p0,self.apex),f2(self.p1,self.apex)])+self.len_offset[1])/f2(_p2,self.apex)
        p2_v=_p2_v*scale
        self.p2=p2_v+self.apex
        r_new=f2(self.apex,self.p2)
        origin_new=self.apex
        z_v_new=f3(np.zeros(3),(self.cross_pt-side_center))
        x_v_new=f3(np.zeros(3),(side_center-self.apex))
        y_v_new=np.cross(z_v_new,x_v_new)
        T_new=f1(x0_v,y0_v,z0_v,x_v_new,y_v_new,z_v_new)
        theta=np.pi/2
        dist_apex_EC=f2(self.apex,side_center)
        dist_apex_p3=f2(self.apex,_p2)
        dist_EC_p3=f2(side_center,_p2)
        angle_EC_A_p3=np.arccos((dist_apex_EC**2+dist_apex_p3**2-dist_EC_p3**2)/2/dist_apex_EC/dist_apex_p3)
        phi=angle_EC_A_p3+angle_offset
        if mirror:phi=-phi
