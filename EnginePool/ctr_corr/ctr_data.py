...
def _update_psic_angles(gonio,scan,point,verbose=True):
    ...
    #
    try:
        if type(scan['eta']) ==float or type(scan['eta']) ==num.float64: 
            eta=scan['eta']
    ...
    #
    try:
        if type(scan['mu']) ==float or type(scan['mu']) ==num.float64: 
    ...
    #
    try:
        if type(scan['nu']) ==float or type(scan['nu']) ==num.float64: 
    ...
    try:
        if type(scan['del']) in [float, num.float64, int]: 
        # if type(scan['del']) ==float or type(scan['del']) ==num.float64: 
        # if type(scan['del']) ==float:
            delta=scan['del']
...
class CtrCorrectionPsic:
    ...
    * A sample description is needed.
    ...
    ##########################################################################
    def ctot_stationary(self,plot=True,fig=None):
        """
